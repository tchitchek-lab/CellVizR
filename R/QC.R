...
computeSmallClusters = function(UMAPdata, 
  ...
  total.cells = apply(data.clusters, 1, FUN = function(x){ifelse((sum(x) < th.size), TRUE, FALSE)})
...
plotSmallClusters = function(values.small) {
  ...
  plot = ggplot2::ggplot(data = data.melted) +
    ...
    ggplot2::geom_tile(ggplot2::aes_string(x ="samples", y="clusters", fill ="small"), colour = "black") +
    ggplot2::scale_fill_manual(values = c("green", "red")) +
    ggplot2::geom_vline(xintercept = (length(unique(data.melted$samples)) - 0.5), colour = "black", size = 2)
...
computeUniformClusters <- function(UMAPdata,
  ...
  for (cluster in gtools::mixedsort(unique(UMAPdata@identify.clusters))) {
    for (marker in colnames(UMAPdata@matrix.expression)) {
      matrix.expression_sub = UMAPdata@matrix.expression[UMAPdata@identify.clusters==cluster,marker]
      diptest <- diptest::dip.test(matrix.expression_sub)$p.value
      quantiles  <- stats::quantile(matrix.expression_sub)
      IQR       <- quantiles[4] - quantiles[2]
      mode      <- computemode(matrix.expression_sub)$y
  ...
  diptests.m = reshape::melt(diptests)
  colnames(diptests.m) = c("clusters","markers","pv_dip")
  
  IQRs$clusters = rownames(IQRs)
  IQRs_m = reshape::melt(IQRs)
  colnames(IQRs_m) = c("clusters","markers","IQR")
  
  res = merge(diptests.m,IQRs_m)
  ...
  res$clusters = factor(res$clusters, levels = rev(gtools::mixedsort(unique(res$clusters))))
...
#' @return a ggplot2 object
#' 
plotUniformClusters <- function(values_uniform){
  ...
  plot = ggplot2::ggplot(data = res) +
    ggplot2::ggtitle(bquote(atop(.(title), atop(italic(.(subtitle)), "")))) +
    ggplot2::geom_tile(ggplot2::aes_string(x ="markers", y="clusters", fill ="passed"), colour = "black") +
